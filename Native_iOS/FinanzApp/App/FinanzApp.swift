...
struct MainTabView: View {
    ...
    private var tabBar: some View {
        HStack(spacing: 0) {
            ...
            TabItem(icon: "list.bullet", label: L.tabMovements, isActive: selectedTab == 1, isDark: vm.isDarkMode, namespace: tabIndicator)
                .onTapGesture { select(1) }
            TabItem(icon: "arrow.triangle.2.circlepath", label: L.tabRecurring, isActive: selectedTab == 2, isDark: vm.isDarkMode, namespace: tabIndicator)
                .onTapGesture { select(2) }
            TabItem(icon: "gearshape.fill", label: L.tabSettings, isActive: selectedTab == 3, isDark: vm.isDarkMode, namespace: tabIndicator)
